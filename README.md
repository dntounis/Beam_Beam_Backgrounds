...
Companion codebase for the paper **Beam-Beam Backgrounds for the Cool Copper Collider** by Dimitrios Ntounis (corresponding author), Caterina Vernieri, Lindsey Gray, Elias Mettner, Emilio A. Nanni, and Tim Barklow.  
...
- `SimHits/` - Snapshot (October 2025) of the EDM4hep timing and hit-rate analysis suite. Contains Python scripts that glob detector outputs, build cached histograms, extract constant-fit rates, and publish the timing figures used in the paper (one-BX vs. train, IPC vs. HPP separation, and subdetector overlays).
...
- `Simulation_flowchart.png` / `Simulation_flowchart.pdf` - Visual summary of the full simulation and analysis pipeline referenced above.
